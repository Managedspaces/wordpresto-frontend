...
The site is being translated into `pt`, `pt-br`, `es`, `de` and `fr` alongside English — see `context.md` for the full plan. The homepage, the waitlist form, `/specialists/*` (the hub and all four team pages), `/workers/` + `/workers/seo/` (the two Worker directory pages), `/prestobot/` and `/workers/[slug]/` (all 20 Worker profile pages, ~700 translated pages in total) are translated so far. Locale-keyed data files live in `src/data/i18n/`: `common.ts` (shared nav/footer strings), `home.ts` (homepage copy), `waitlist.ts` (waitlist form copy), `specialists.ts` (specialists hub + team page copy), `workersDirectory.ts` (Worker directory page chrome), `prestobot.ts` (WordPrestoBot crawler page copy), `workerProfilePage.ts` (profile-page chrome shared across all 20 workers) and `workerProfiles/` (per-worker translated content, one file per locale per half of the roster — `{locale}.batch1.ts` / `{locale}.batch2.ts` — merged onto the English `workerProfiles.ts` record via `getLocalizedProfile()`, the same "don't duplicate English" pattern as `getTeamMeta()` below). The specialist-bench team names/summaries (`src/data/workerRegistry.ts`) are translated too, via `getTeamMeta(id, locale)` / `getTeamGroups(max, locale)`. On Worker directory cards and profile pages, `role`, `department`, `stage`, `team` and the `heroTagPills` values stay English everywhere — this is the same fixed worker taxonomy shown consistently across every page a worker appears on, so it stays legible/searchable regardless of locale; only the descriptive prose around it translates. `/sitemap/` stays English-only by design — it's a machine-oriented route index that already lists every locale's URLs, so translating it would mean 5 more copies of the same ~75-entry list with no real reader value; see `context.md` for the full reasoning. Technical literals (User-Agent strings, `robots.txt` syntax, product/bot names, internal action tokens) never translate — only surrounding prose does. When translating a new page, follow the homepage's pattern: extract every visible string into a `Record<Locale, ...>` dictionary in `src/data/i18n/`, parameterise a shared component in `src/components/` by `locale`, and add a thin `src/pages/[locale]/{page}.astro` route alongside the existing English one. If near-duplicate pages exist per entity (like the four `/specialists/*` team pages, which differed only by team id and two short strings), consolidate them into one component parameterised by both `locale` and the entity id, routed via a single dynamic `src/pages/[locale]/{page}/[entity].astro` — don't multiply near-identical `.astro` files across locales. For pages with client-side scripts that need locale strings (like the waitlist form), serialise them into a `<script type="application/json">` tag rendered server-side and parse it at script init, rather than duplicating the script per locale — see `WaitlistPage.astro` (`#wl-i18n`) for the pattern. Waitlist form option `value`s stay in English across every locale (only the `label` translates) so data in Postgres and the Resend notification email stay analysable in one language. For very large per-entity translation volumes (20 Worker profiles × 5 locales), split the work across parallel background agents by locale and by a manageable chunk of entities (10 workers each worked well) rather than one single-shot translation — each agent writes its own batch file, reviewed and merged centrally. Before shipping any locale page, do a real Playwright screenshot pass, not just a string-audit grep — two site-wide gaps (hardcoded "How it works" footer link, hardcoded Worker-profile quad-card headings) were only caught by looking at a live rendered page.
